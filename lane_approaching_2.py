...
class RadarSensor(object):
	def __init__(self, parent_actor):
		...
		self.sensorbackright = world.spawn_actor(bp, carla.Transform(carla.Location(x=-2.5, y=1.0, z=1.0), carla.Rotation(yaw=155,)), attach_to=self._parent)
		self.sensorbackleft = world.spawn_actor(bp, carla.Transform(carla.Location(x=-2.5, y=-1.0, z=1.0), carla.Rotation(yaw=-155)), attach_to=self._parent) 
...
#
	...
	print(bp4)
	spawn_point4 = carla.Transform(carla.Location(-355, 36.9, 1), carla.Rotation(0,0,0))
...
#				current_lane = "right"
		...
		try:
			...
			if detector_notify["front"] != 0:
				...
				print('back right velo: ' + str(detectright.velocity/math.cos(0.436)))
		...
		#	toggle_radar()
			...
			traffic_manager.vehicle_percentage_speed_difference(obstacle4, 50)
		elif keys[K_z]:
