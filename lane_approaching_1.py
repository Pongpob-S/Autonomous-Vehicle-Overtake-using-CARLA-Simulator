...
def carlaToFile(actor, filename, detector):
	...
	fieldnames = ['velocity','detector_velocity','detector_depth','x', 'y', 'z']
	rows = [{'velocity':absolute_velocity,'detector_velocity': str(detector.velocity), 'detector_depth': detector.depth, 'x': veh_x,'y': veh_y, 'z': veh_z}]
...
def obToFile(actor, filename):
	...
	fieldnames = ['velocity','x', 'y', 'z']
	rows = [{'velocity':absolute_velocity,'x': veh_x,'y': veh_y, 'z': veh_z}]
...
def drawDetectorStatus(image):
	...
	custom_fontscale = 0.8
	image = cv2.putText(image, f"Obstacle detector", (20, 530), GLOBAL_FONT, custom_fontscale, custom_color, custom_thickness)
	image = cv2.putText(image, f"front: {detector_notify['front']} m", (20, 560), GLOBAL_FONT, custom_fontscale, custom_color, custom_thickness)
	image = cv2.putText(image, f"back: {detector_notify['back']} m", (20, 590), GLOBAL_FONT, custom_fontscale, custom_color, custom_thickness)
	image = cv2.putText(image, f"left: {detector_notify['left']} m", (20, 620), GLOBAL_FONT, custom_fontscale, custom_color, custom_thickness)
	image = cv2.putText(image, f"right: {detector_notify['right']} m", (20, 650), GLOBAL_FONT, custom_fontscale, custom_color, custom_thickness)
	image = cv2.putText(image, f"frontleft: {detector_notify['frontleft']} m", (20, 680), GLOBAL_FONT, custom_fontscale, custom_color, custom_thickness)
	image = cv2.putText(image, f"frontright: {detector_notify['frontright']} m", (20, 710), GLOBAL_FONT, custom_fontscale, custom_color, custom_thickness)
...
def timeToCollision(detector):
	
	time = (detector.depth) / (math.cos(0.436)) / (abs(detector.velocity) / math.cos(0.436))
...
class RadarSensor(object):
	def __init__(self, parent_actor):
		...
		bp.set_attribute('vertical_fov', str(0))
		...
		self.sensorbackright = world.spawn_actor(bp, carla.Transform(carla.Location(x=-2.5, y=1.0, z=1.0), carla.Rotation(yaw=170,)), attach_to=self._parent)
		self.sensorbackleft = world.spawn_actor(bp, carla.Transform(carla.Location(x=-2.5, y=-1.0, z=1.0), carla.Rotation(yaw=-170)), attach_to=self._parent) 
...
#
	...
	#spawn_point2 = carla.Transform(carla.Location(-333.4, 36.9, 1),carla.Rotation(0,0,0))
	...
	#spawn_point3 = carla.Transform(carla.Location(-332.8, 30.1, 1),carla.Rotation(0,0,0))
	...
	print(bp4)
	spawn_point4 = carla.Transform(carla.Location(-380, 36.9, 1), carla.Rotation(0,0,0))
	obstacle4 = world.spawn_actor(bp4, spawn_point4)
	actor_list.append(obstacle4)

	bp5 = blueprint_library.filter('cybertruck')[0]
	print(bp5)
	spawn_point5 = carla.Transform(carla.Location(-355, 30.1, 1), carla.Rotation(0,0,0))
	...
	while not GameDone:
		...
		try:
			...
			front_car_speed = obstacle.get_velocity()
			...
			if detector_notify["front"] != 0:
				...
				carlaToFile(vehicle, 'proj_vehicle.csv', detectright)
				obToFile(obstacle, 'proj_obstacle.csv')
				obToFile(obstacle4, 'proj_obstacle4.csv')

				print('Time to collision: ' + str(timeToCollision(detectright)))
				print('back right distance: ' + str(detectright.depth))
				print('back right velo: ' + str(detectright.velocity))
				...
				if current_lane == "changable":
					if detector_notify["frontright"] != 0 or detector_notify["right"] != 0:
						if detector_notify["frontleft"] != 0 or detector_notify["left"] !=0:
							...
							if timeToCollision(detectleft) > 4.24:
								use_lane_detect = False
								...
								vehicle.enable_constant_velocity(front_car_speed)
								print("rear left vehicle is approaching")
					elif keys[K_d]:
						if timeToCollision(detectright) > 4.24:
							use_lane_detect = False
							car_control.steer = 0.3
							car_control.brake = 0.0
							vehicle.disable_constant_velocity()
							...
							vehicle.enable_constant_velocity(front_car_speed)
							print("rear right vehicle is approaching")
		...
		#	toggle_radar()
			...
			print("rear right upcoming")
			obstacle4.set_autopilot(True)
			traffic_manager.vehicle_percentage_speed_difference(obstacle4, 80)
		elif keys[K_z]:
			print("rear left upcoming")
			obstacle5.set_autopilot(True)
			traffic_manager,vehicle_percentage_speed_difference(obstacle5, 50)
